...
void Tube::buildMesh( ci::TriMesh *tubeMesh  )
{
    ...
    std::vector<uint32_t> indices = (*tubeMesh).getIndices();
    std::vector<Vec3f> verts = (*tubeMesh).getVertices();
    
    for(size_t i=0;i<verts.size();++i)
        normals.push_back( Vec3f::zero() );
    
    for(size_t i=0;i<(*tubeMesh).getNumTriangles();++i)
    {
        Vec3f v0 =verts[ indices[i * 3] ];
        Vec3f v1 = verts[ indices[i * 3 + 1] ];
        Vec3f v2 = verts[ indices[i * 3 + 2] ];
    ...
    {
        normals[i].normalize();
        (*tubeMesh).appendNormal(normals[i]);
    }
     */
}
